...
class TestUpdateStaff(TestStaffSetup):
    ...
    def test_if_user_is_anynomous_then_return_401(self):
        ...
        response = self.client.put(f"{self.staff_url}/{staff.id}", self.staff, format='json')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_if_data_is_invalid_then_return_400(self):
        # self.force_authenticate(user=Account(is_staff=True))
        staff = baker.make(Staff)
        response = self.client.put(f"{self.staff_url}{staff.id}", {"username": "test"}, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST) 
...
class TestListStaff(TestStaffSetup):
    ...
    def test_if_fields_is_valid_return_200(self):
        # self.force_authenticate(user=Account(is_staff=True))
        response =self.client.get(f"{self.staff_url}?fields={self.field_params}", format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_if_sort_is_valid_return_200(self):
        # self.force_authenticate(user=Account(is_staff=True))
        response =self.client.get(f"{self.staff_url}?sort={self.sort_params}", format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class TestSearchStaff(TestStaffSetup):
    ...
    @pytest.mark.skip(reason="not implemented")
    def test_if_user_is_anynomous_then_return_401(self):
        response = self.client.post(f"{self.staff_url}search", self.filter_params, format='json')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_if_filter_params_is_invalid_return_400(self):
        # self.force_authenticate(user=Account(is_staff=True))
        response =self.client.post(f"{self.staff_url}search", {"filter_params": "test"}, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_if_fields_is_valid_return_200(self):
        # self.force_authenticate(user=Account(is_staff=True))
        response =self.client.post(f"{self.staff_url}search?fields={self.field_params}", format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_if_fields_is_invalid_return_400(self):
        # self.force_authenticate(user=Account(is_staff=True))
        response =self.client.post(f"{self.staff_url}search?fields=test", format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_if_sort_is_valid_return_200(self):
        # self.force_authenticate(user=Account(is_staff=True))
        response =self.client.post(f"{self.staff_url}search?sort={self.sort_params}", format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
