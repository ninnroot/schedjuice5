...
class NameGuesser(object):
    ...
    def guess_format(self, name):
        ...
        if name in ("username", "login", "nickname", "name", "owner_name"):
            return lambda x: faker.user_name()
        if name in ("email", "email_address", "primary_email", "secondary_email"):
        ...
        if name == "password":
            return lambda x: make_password(faker.password())
        if name == "gender":
            return lambda x: random.choice(["male", "female"])
        if name == "start_date":
            return lambda x: datetime.now().date()
        if name == "end_date":
            return lambda x: datetime.now().date() + timedelta(days=7)
        if name in ("time_from", "opening_time"):
            return lambda x: datetime.now().time()
        if name in ("time_to", "closing_time"):
            return lambda x: (datetime.now() + timedelta(hours=2)).time()
